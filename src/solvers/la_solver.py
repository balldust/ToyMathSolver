...
class LinearEquationSolver:
    ...
    def _perform_gaussian_elimination(self):
        for row1 in range(self._nrows):
            for row2 in range(row1 + 1, self._nrows):
                ...
                multiplier = (
                    self._extended_multiplier_matrix[row2][row1] / pivot_element
                )
                for col in range(row1, self._ncols):
                    self._extended_multiplier_matrix[row2][col] = (
                        self._extended_multiplier_matrix[row2][col]
                        - multiplier
    ...
    def _perform_pivoting(self, init_row):
        ...
        current_pivot_element = self._extended_multiplier_matrix[
            current_pivot_row
        ][init_row]
        for row in range(init_row, self._nrows):
            pivot_element_to_check = self._extended_multiplier_matrix[row][
                init_row
            ]
            if math.fabs(pivot_element_to_check) > math.fabs(
                current_pivot_element
            ):
    ...
    def _reassign_rows_based_on_pivoting(self, init_row, current_pivot_row):
        if current_pivot_row != init_row:
            ...
            self._extended_multiplier_matrix[
                init_row
            ] = self._extended_multiplier_matrix[current_pivot_row]
    ...
    def _get_variable_values_by_backward_substitution(self):
        ...
        for row in range(self._nrows - 2, -1, -1):
            val_to_add = (
                self._extended_multiplier_matrix[row][-1]
                / self._extended_multiplier_matrix[row][row]
            )
            for col in range(row + 1, self._ncols - 1):
                val_to_add = (
                    val_to_add
                    - self._extended_multiplier_matrix[row][col]
                    * calculated_vars[col]
                    / self._extended_multiplier_matrix[row][row]
                )
    ...
    def _calculate_last_variable(self, calculated_vars):
        if self._extended_multiplier_matrix[-1][-2] != 0:
            calculated_vars[-1] = (
                self._extended_multiplier_matrix[-1][-1]
