...
class NonLinearEquationSolver:
    def __init__(
        ...
        nl_solver_specs: NonLinearSolverSpecifications,
    ):
    ...
    def _solve_newton_raphson(self) -> bool:
        ...
        while (
            max(
                [
                    math.fabs(res)
                    for res in self._equation_system.get_equation_evaluations()
                ]
            )
    ...
    def _construct_jacobian(self):
        ...
        jacobian = [[0 for _ in range(n_vars + 1)] for _ in range(n_equations)]
        return jacobian
