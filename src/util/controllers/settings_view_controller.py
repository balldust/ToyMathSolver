...
from src.util.factories.property_factory import (
    PropertyFactory,
    NonSettablePropertyFactory,
)
...
class SettingsViewController:
    ...
    def _add_non_linear_solver_specifications(self, root: TreeNode, child: int):
        ...
        root.child(child).append_child(
            TreeItem(
                solver_specs_factory.JACOBIAN_PERTURBATION, solver_specs_factory
            )
        )
        root.child(child).append_child(
            TreeItem(
                solver_specs_factory.MAXIMUM_ITERATIONS, solver_specs_factory
            )
        )
        root.child(child).append_child(
            TreeItem(
                solver_specs_factory.CONVERGENCE_TOLERANCE, solver_specs_factory
            )
        )
